...
which python
if [ $? -eq 0 ]; then
	python xml2po.py $(find $findoptions ../usr/lib/ -name "*.xml") ../usr/share/enigma2/MetrixHD/skinTemplates.xml > MyMetrixLite-xml.pot
else
	python3 xml2po.py $(find $findoptions ../usr/lib/ -name "*.xml") ../usr/share/enigma2/MetrixHD/skinTemplates.xml > MyMetrixLite-xml.pot
fi
